...
def main():
    ...
    try:
        ...
        print(f"Connected. Will keep listening for 180s to collect fortune screenshot.")
        ...
        while (time.time() - start_time) < 180:
                    ...
                    if base64_data:
                        try:
                            ...
                            if os.path.exists(latest_path):
                                ...
                                backup_path = f"screenshots/history/screenshot_{timestamp}.jpg"
                                os.rename(latest_path, backup_path)
                                print(f"Backed up old: {backup_path}")
                            with open(latest_path, 'wb') as f:
                                f.write(jpeg_data)
                            print(f"SAVED: {latest_path} ({len(jpeg_data)} bytes)")
                            # 保存后立刻退出，避免反复
                            print("Done!")
                            return
            ...
            if elapsed - last_progress >= 10:
                ...
                print(f"\n[{elapsed}s elapsed, idle {idle}s, {received_bytes} bytes received, {screenshot_count} screenshots started]")
                last_progress = elapsed

        print(f"\nTimeout. Total: {screenshot_count} screenshots started, {received_bytes} bytes")
