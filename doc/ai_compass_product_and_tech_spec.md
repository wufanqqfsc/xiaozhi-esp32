...
> **📋 文档状态 (v1.12 · 2026-06-15)**:
> - **【已实现】** 太极+鱼眼 **90% 缩放**（R=86 / 鱼眼 28px）、L4 外圈 r=179（直径 358、3px）；**已移除** Layer2 次外鎏金细环。
> - **【已实现】** **12 项运势功能图标环**（Font Awesome **35px** 视觉、`font_awesome_30_4` + 缩放）；**环形触摸层**按极角分扇区选中（r≈82~179）；图标仅展示，**Boot 短按**切换选中、**长按**触发运势；选中音 `OGG_POPUP`。
> - **【已实现】** 常态太极 **60s/圈**；运势 **Animating 20s**（每 4s 一档 0.2→0.4→0.4→0.4→0.2 圈）；恢复常态 **1s** 后出结果卡。
> - **【已实现】** CST816S 触控初始化；板级音频改为 **BoxAudioCodec**（ES8311 + ES7210，PA=GPIO9）；`Application::PlayUiSound()` 统一 UI 提示音路径。
> - **【已实现】** 开机阶段防白闪：`panel_init` 后铺 RGB565 `0x0841`；`screen` / `round_mask` 设 `LV_OPA_COVER`。
> - **【待验收】** 扬声器/麦克风真机出声；开机白闪是否完全消除。
> - **【规划中】** 真实 BLE 驱动、语音触发运势、服务端 Fortune MCP。
> - **【验收清单】** 见 [`ai_compass_iteration_acceptance.md`](ai_compass_iteration_acceptance.md)。
...
| 外设 | 板卡硬件 | 固件已集成 | 说明 |
|-----|---------|-----------|------|
| ST77916 QSPI LCD 360×360 | ✅ | ✅ | `AttitudeDisplay` + `CompassTaiji`；RST=GPIO3；开机铺深色底防白闪 |
| ES8311 + ES7210 编解码 | ✅ | ✅ | `BoxAudioCodec`（I2C + I2S，PA=GPIO9）；**真机出声待验收** |
| WiFi | ✅ | ✅ | `WifiBoard` 基类 |
| CST816S 触控 | ✅ | ✅ | `InitializeTouch()` + LVGL touch；环形运势选择 |
| Boot / 电源按键 | ✅ | ✅ | Boot：运势选择/触发 + 对话切换；PWR：背光/电源控制 |
| 背光 PWM | ✅ | ✅ | `PwmBacklight` GPIO5 |
| QMI8658 IMU | ✅ | ❌ | `SetAttitudeData()` API 预留，UI 未消费 |
| BQ27220 电量计 | ✅ | ❌ | 未接入板级驱动 |
| PCF85363 RTC | ✅ | ❌ | 未接入板级驱动 |
| Bluetooth BLE | ✅ | 🟡 | `CONFIG_XIAOZHI_ENABLE_BLE_FISHEYE`；鱼眼 UI 已有，真实连接态驱动待迭代 3 |
...
| **交互方式** | 语音唤醒 + 自然语言对话 + **触控运势环** + Boot 键（短按切换 / 长按触发） |
...
- **炫酷的视觉交互**: 动态旋转的太极图（60秒/圈） + 鎏金色外圈 + **12 项运势功能图标环** + 运势动画
- **流畅的语音对话**: 基于 Xiaozhi AI 协议的端到端实时语音交互（ES8311/ES7210 编解码，待真机验收）
...
├── 🎨 **罗盘显示系统** ⭐ 本项目核心
│   ├── 动态太极图 (60秒旋转 + 4px 鎏金边)
│   ├── L0 太极 + L1 核心容器 + **L5 运势功能环（12 图标）** + L4 外圈
│   └── WiFi/BLE 鱼眼（与太极一体旋转）
│
├── 🔧 **设备管理系统**
...
- 实时打断：语音播放时可随时打断
- 流式响应：边说边听，无需等待完整回复
- 本板音频：`BoxAudioCodec`（ES8311 播放 + ES7210 双麦 AEC，见 §1.1.1）；UI 提示音经 `Application::PlayUiSound()` → `AudioService::PlaySound()`
...
| 层级 | 元素 | 半径范围 | 行为 | 技术实现 |
|-----|-----|---------|-----|---------|
| L0 中心 | 太极图 + WiFi/BLE 鱼眼 | 0–**86** | **60s/圈** 容器整体旋转 | canvas **172×172** + 鱼眼 **28×28×2** |
| L1 内层 | 核心信息容器 | 0–86 | 固定 | 透明容器，与太极同径 |
| ~~L2~~ | ~~次外鎏金细环~~ | — | **已移除** | 原 r=143 |
| L4 外圈 | 鎏金边框 | **179** | 固定 | LVGL arc **3px** 鎏金，**358×358** |
| L5 功能环 | **12 项运势图标** | **129**（环心半径） | Idle 可见；Animating/Result 隐藏 | Font Awesome **35px** 视觉；选中 **1.1×** 缩放 + 高亮 |
| 触摸层 | 环形扇区选择 | r **82~179** | Idle 置顶透明层 | `FortuneMenuIndexFromPoint()` 极角分 12 扇区 |
| 背景 | 中心暗色圆盘 | **135** | 固定 | `bg_layer_center_` 270×270 填色（非线环） |

**当前可见「线环」**（由内到外）：① 太极 canvas 鎏金边 **4px** → ② **12 运势图标环**（r≈129）→ ③ L4 外圈 r=179（直径 358，3px）。

**12 项运势功能**（`FortuneMenuType`，见 `doc/功能 图标.md`）:
...
**运势环交互**（Idle 态）:
1. **触控**：整屏透明 `fortune_menu_ring_touch_` 置顶；点击环带内按极角映射扇区 → 选中对应图标 + 播放 `OGG_POPUP`
2. **Boot 短按**：首次进入选中态（默认「今日运势」）；再次短按 **循环切换** 12 项
3. **Boot 长按**：对当前选中项调用 `ShowFortuneFromMenu()` → Animating → Result
4. 图标 label **不可点击**（`LV_OBJ_FLAG_CLICKABLE` 已清除），避免与环形触摸层冲突
5. **已移除** 图标下方功能名字文字（仅保留图标）

**太极图绘制算法** ([compass_taiji.cc](../main/display/compass_taiji.cc)):
1. **外径联动鱼眼**：`TAIJI_RADIUS=86`，`FISHEYE_ICON_SIZE=28`，canvas **172×172** ARGB8888（约 **118KB** PSRAM）
2. **鱼眼替代 canvas 圆点**：由 28×28 WiFi/BLE 图标叠加
3. **一体旋转**：`lv_obj_set_style_transform_rotation(taiji_container_, …)`；**LVGL 定时器** 150ms 步进（非 FreeRTOS 任务），常态 **60s/圈**
4. **阳鱼/阴鱼/互抱**：经典算法；canvas 上 **4px** 鎏金外描边；旋转时关闭 canvas 抗锯齿以减轻刷屏

**罗盘组件层级（真实代码结构 · v1.12）**:
```
screen (360×360, bg_opa=COVER)
├── round_mask (360×360 圆裁切)
└── attitude_container_
    ├── background_ → bg_layer_center_ (270×270 中心暗色圆)
    ├── layer1_container_ (172×172 透明，与太极同径)
    ├── layer4_outer_ring_ (358×358, r=179, 3px 鎏金)
    ├── fortune_menu_labels_[12] (r≈129 环上 Font Awesome 图标，仅展示)
    ├── fortune_menu_ring_touch_ (360×360 透明，环形触摸置顶)
    └── taiji_container_ (172×172, r=86, 60s/圈)
        ├── canvas (阴阳鱼 + 4px 金边)
        ├── taiji_gold_pulse_arc_ (运势 Animating 金边脉冲，默认隐藏)
        ├── wifi_fisheye (28×28, local 80,32)
        └── ble_fisheye (28×28, local 80,128)
```

**实现文件**:
- [compass_taiji.cc](../main/display/compass_taiji.cc) - 太极图（LVGL 定时器自动旋转 60s/圈）
- [attitude_display.h](../main/display/attitude_display.h) - 罗盘显示参数（半径宏定义）
- [attitude_display.cc](../main/display/attitude_display.cc) - 罗盘 UI 布局
...
│   - GetAudioCodec() → BoxAudioCodec (ES8311 + ES7210, PA=GPIO9) │
...
> **本板当前实现**：音频走 `BoxAudioCodec`（ES8311 播放 + ES7210 录音/AEC，I2C GPIO10/11，I2S MCLK/BCLK/LRCK/DOUT/DIN 见 `config.h`）。UI 提示音：`PlayUiSound()` 投递主循环后 `AudioService::PlaySound()` 自动 `EnableOutput(true)`。

```
  用户声音 (双麦克风 → ES7210 → I2S DIN GPIO39)
    │
    ▼
┌──────────────────┐
...
└────────┬─────────────────┘
         ...
         ▼
┌──────────────────┐
│  Application     │  ← ES8311 → I2S DOUT GPIO47 → PA GPIO9
│  - 音频解码      │
...
│   ├── compass_taiji.h/cc        # ⭐ 太极图（172×172 canvas, r=86, 60s/圈 + 鱼眼）
...
> 本章描述运势与 BLE 等 **Target2 能力**。环上八卦名/卦象/方位大字属 Target1，仅见 [附录 A](#附录-a-target1-显示设计参考未实现)，**不再实施**。

> **状态 (v1.12)**：运势三态 + 结果卡 + **12 项功能图标环** + 环形触摸/Boot 交互 **【已实现】**；服务端 Fortune MCP 与语音端到端联调 **【规划中】**。
...
| 能力 | 状态 | 实现要点 |
|-----|------|---------|
| Idle / Animating / Result 三态 | ✅ | `FortuneState` + 计时器 |
| 200×240 结果卡（7 行） | ✅ | `CreateFortuneCard()` |
| **12 项功能图标环** | ✅ | `CreateFortuneMenuRing()`，`FORTUNE_MENU_RING_RADIUS≈129` |
| **环形触摸选择** | ✅ | `fortune_menu_ring_touch_` + `FortuneMenuIndexFromPoint()` |
| **Boot 短按 / 长按** | ✅ | `HandleBootKey()` / `HandleFortuneBootLongPress()` |
| 选中提示音 | ✅ | `PlayFortuneMenuSelectSound()` → `PlayUiSound(OGG_POPUP)` |
| 激活成功音 | ✅ | `PlayUiSound(OGG_SUCCESS)` |
| 内置演示数据 | ✅ | `kFortuneMenuDefs[12]` 含卦名/解读/宜忌 |
| 语音 / MCP 自动触发 | ⬜ | 阶段五 |

#### 4.1.2 方案 A：服务端 MCP 工具 + 客户端 UI 扩展（规划中）
...
2. 客户端 (Target2): 扩展 AttitudeDisplay
   ├─ CreateWifiFisheye / CreateBleFisheye → **28×28** 鱼眼叠在 taiji_container_ 鱼眼位
   ├─ CreateFortuneMenuRing() → **12** Font Awesome 图标环（**35px** 视觉）
   ├─ CreateFortuneMenuRingTouch() → 环形扇区触摸 + Boot 键切换/触发
   ├─ EnterAnimatingState() → **20s**：每 **4s** 一档太极 **0.2→0.4→0.4→0.4→0.2 圈** + 亮度/**太极金边**/鱼眼同步；L4 外圈静态；**恢复 60s/圈后 1s** 出结果卡
   ├─ ShowFortune(...) + CreateFortuneCard() → 200×240 胶囊结果卡（7 行）
   └─ 语音触发见 §7.5（阶段五），当前支持触摸/Boot/手动 `ShowFortune()` 演示
...
**防白闪**（仅影响上电至 LVGL 首帧阶段）:
1. `esp_lcd_panel_init()` 后、`disp_on_off(true)` 前，逐行 `draw_bitmap` 填充 RGB565 **`0x0841`**（与 `SpiLcdDisplay` 玄黑底一致）
2. `AttitudeDisplay::SetupUI()` 中 `screen`、`round_mask`、`attitude_container_` 设置 `bg_opa = LV_OPA_COVER`
3. LVGL 绘制缓冲保持 **72 行**（勿盲目加大，曾导致黑屏/起不来）

**触控**（CST816S）:
- I2C `GPIO10/11`，RST=`GPIO1`，INT=`GPIO4`
- `lvgl_port_add_touch()` 注册；运势环另用透明全屏层处理扇区命中，不依赖单图标点击
...
2. Application 启动后调用 AttitudeDisplay::SetupUI()（仅执行一次）
   ...
   ├─ CompassTaiji::Create(cx=180, cy=180, r=86)  # FISHEYE 28px → R=86
   │   └─ 172×172 ARGB8888 canvas；鱼眼位由 WiFi/BLE 图标填充
   ├─ CreateWifiFisheye / CreateBleFisheye on taiji_container_
   ├─ CompassTaiji::StartAutoRotation(60000)  # 容器整体旋转 60s/圈
   ├─ CreateLayer1CoreInfo() → 透明核心容器 172×172 (r=86)
   ├─ CreateFortuneMenuRing() → **12** 图标环 (r≈129, Font Awesome 35px 视觉)
   ├─ CreateLayer4Boundary() → 外圈边框 358×358, 3px 鎏金 (r=179)
   └─ CreateFortuneMenuRingTouch() → 360×360 透明环形触摸层（置顶）
```
...
└── attitude_container_ (360×360)
    ...
    ├── bg_layer_center_ (270×270, 0x121212)
    ├── compass_taiji (Layer0, 172×172 canvas, r=86, 60s/圈容器旋转)
    │   ├── wifi_fisheye (28×28, local 80,32)
    │   └── ble_fisheye (28×28, local 80,128)
    ├── layer1_container_ (172×172 透明核心区, r=86)
    ├── layer4_outer_ring_ (358×358, r=179, 3px 鎏金外边框)
    ├── fortune_menu_labels_[12] (环 r≈129, 35px 图标)
    └── fortune_menu_ring_touch_ (透明全屏，环形扇区)
```

### 5.3 罗盘参数宏（【已实现】）

来源: [attitude_display.h](../main/display/attitude_display.h)

```c
#define SCREEN_W                  360
#define SCREEN_H                  360
#define LAYER4_BOUNDARY_RADIUS    179   // 外圈直径 358
#define TAIJI_RADIUS              86
#define FISHEYE_ICON_SIZE         28
#define TAIJI_CANVAS_SIZE         172
#define FORTUNE_MENU_COUNT        12
#define FORTUNE_MENU_ICON_GLYPH_PX 35
#define FORTUNE_MENU_RING_RADIUS  129   // (86+179)/2 - 3
#define FORTUNE_MENU_TOUCH_INNER_R 82   // TAIJI_RADIUS - 4
#define FORTUNE_MENU_TOUCH_OUTER_R 179
#define FORTUNE_MENU_START_ANGLE_DEG (-90)  // 12 点钟起顺时针
#define TAIJI_ROTATION_PERIOD_NORMAL_MS  60000
#define FORTUNE_ANIM_DURATION_MS  20000
#define FORTUNE_TAIJI_PHASE_MS     4000
#define FORTUNE_RESULT_CARD_DELAY_MS 1000
```
...
|---------|------|------------------------|------|
| 太极图 + 鱼眼 | **60 秒/圈** | 3600 / (60000/150) = **9** (0.4°/step) | `taiji_container_` transform + LVGL timer 150ms ✅ |
| WiFi/BLE 鱼眼 | **随太极旋转** | — | `taiji_container_` 子对象 **28×28** |
| 运势功能环 | **固定不转** | — | 12 图标随 `attitude_container_` 静止；Animating/Result 隐藏 |
...
> **更新说明**：阶段一、阶段二 **【已实现】**；阶段三至六为 **【规划中】**；运势视觉按 **Target2**（同心圆 + 结果卡）实施，Target1 环上八卦仅作附录参考。每阶段以 **先视觉骨架 → 再真实驱动 → 最后联调** 推进，经 `build_and_flash.sh` / `build_and_flash.ps1` 验证。
>
...
| 罗盘布局 (Target2) | ✅ 已完成 | `main/display/attitude_display.cc` | L0 太极 / L1 / L4 外圈（L2 已移除） |
| 太极图 + 鱼眼 | ✅ 迭代1 完成 | `compass_taiji.cc` + `attitude_display.cc` | **r=86** + **28px** 鱼眼共旋转 + **4px** 鎏金外圈 |
| ~~内圈细环 (Layer2)~~ | ❌ 已移除 | — | 原 r=143，2026-06-15 去掉 |
| 外圈金色边框 (Layer4) | ✅ 已完成 | `main/display/attitude_display.cc` | **r=179**（358×358）, **3px** 鎏金边 |
...
| ~~3 圈环形边界 (r=72/100/145)~~ | ❌ **未实现** | — | Target1 设计 |
| ~~八卦环自动旋转 (45s)~~ | ❌ **未实现** | `UpdateBaguaPositions()` 代码已注释 | 八卦已移除, 旋转逻辑保留但未调用 |

**当前验收**：上电 → 太极 **172×172**（R=86）**60s/圈** → **12 运势图标环**可见 → **28×28** WiFi/BLE 鱼眼随太极转 → L4 外圈 **r=179** **3px** 鎏金 → 触摸环带/Boot 可选功能并触发运势流程（2026-06-15 迭代）

---

### 阶段二：视觉框架扩展 — 鱼眼状态图标（✅ 【已完成 · v1.7 一体旋转】）

**验收日期**: 2026-06-15（见 [`ai_compass_iteration_acceptance.md`](ai_compass_iteration_acceptance.md) §5）

**目标**：WiFi / BLE **28×28** 圆形图标作为太极阴阳鱼的鱼眼，与太极 **一体旋转**（**60s/圈**）。

**★ 关键设计（v1.9）**：
- **一体旋转**：鱼眼在 `taiji_container_` 内；LVGL 定时器 **150ms** 步进 transform
- **尺寸**：鱼眼 **28×28**；`TAIJI_RADIUS = 86`，canvas **172×172**
- **局部坐标**（相对容器）：上眼 WiFi `(80, 32)`，下眼 BLE `(80, 128)`
- **BLE 鱼眼样式**：**白底** + **边框**区分状态（灰边 / 灰白脉冲 / 蓝边 `#2196F3`）；WiFi 仍为金/灰 + opacity 脉冲

**开发任务（预估 1 周）**：

| # | 具体任务 | 修改文件 | 验收标准 |
|---|---------|---------|---------|
| 1 | 鱼眼/太极宏 + 状态枚举 | `attitude_display.h` | `FISHEYE_ICON_SIZE=28` / `TAIJI_RADIUS=86` ✅ |
| 2 | 太极 R=86，canvas 鱼眼位留白 | `compass_taiji.cc` | 172×172 canvas ✅ |
| 3 | 容器级旋转 + LVGL timer 150ms | `compass_taiji.cc/.h` | 鱼眼与太极同步 **60s/圈** ✅ |
| 4 | WiFi/BLE 鱼眼叠在 `taiji_container_` | `attitude_display.cc` | local (80,32)/(80,128) ✅ |
| 5 | `UpdateWifiFisheye` / `UpdateBleFisheye` | `attitude_display.cc` | WiFi 金/灰脉冲；BLE **白底+边框** 状态 ✅ |
...
### 阶段三：AI 运势引擎 — 三态状态机 + 功能环 + 结果卡（🟡 【代码已实现 · 真机验收中】）

**目标**：在 Target2 罗盘上实现 Idle / Animating / Result 三态，**12 项运势功能环**（触摸 + Boot 键），配合 200×240 胶囊结果卡。

**★ Target2 设计原则（v1.9）**：
- 所有高亮仅修改颜色/透明度，不修改尺寸和位置
- 鱼眼随太极 **一体旋转**（与阶段二一致）
- **Animating 态（20s）**：每 **4s** 一档太极转 **0.2→0.4→0.4→0.4→0.2 圈**；中心亮度 / **太极金边脉冲** / 鱼眼同步；**L4 外圈保持静态**
- **恢复常态 60s/圈后延迟 1s** 再淡入结果卡（`FORTUNE_RESULT_CARD_DELAY_MS`）
- `HighlightDirection(dir)` / `HighlightGua(idx)` → 结果卡内脉冲（环上方位点已不创建）
- 关闭结果卡：Boot 键 / MCP / 30s 超时
...
| 3 | `EnterAnimatingState()` | `display/attitude_display.cc` | **20s**：5×4s 分段转速 + 同步视觉效果；**恢复 60s/圈 + 1s** 后切 Result ✅ |
...
| 7 | **12 项功能图标环** | `CreateFortuneMenuRing()` | Font Awesome **35px**；环 r≈129；选中 1.1× 缩放 ✅ |
| 8 | **环形触摸 + Boot 交互** | `CreateFortuneMenuRingTouch()` / `HandleBootKey()` | 极角 12 扇区；短按切换、长按触发；`OGG_POPUP` ✅ |
| 9 | 关闭结果卡 | `attitude_display.cc` | Boot 键或 MCP 或 30s 超时 → Idle |
| 10 | 编译 & 真机验证 | `build_and_flash.ps1` | 触摸/Boot 选功能 → Animating **20s** → 结果卡 → Idle；**音频待验收** |

> **范围说明**：语音指令触发运势流程归属 **阶段五**（§7.5），本阶段不包含端到端语音联调。

**核心状态机流程**：
```
Idle → Animating(20s, 每4s 0.2→0.4圈 + 亮度/太极金边/鱼眼同步) → 恢复60s/圈 → 延迟1s → Result(30s) → Idle
```

**资源影响**：
- PSRAM 增量：约 150KB（结果卡 + 子元素 + 动画对象）
- CPU 增量：约 +2~4%（颜色脉冲动画，无八卦旋转）
...
| 2 | BLE 广播/连接集成 | `main/application.cc` 或 `main/ble/ble_server.*` | 启动 BLE → **白底灰边** + 边框脉冲 (ADVERTISING)；连接 → **白底蓝边** `#2196F3` (CONNECTED)；关闭 → 白底灰边 (DISABLED) |
...
| 阶段 | 内容 | 预估时间 | 交付物 |
|-----|-----|---------|-------|
| 一 | 基础罗盘 Target2（太极 + L4 外圈） | ✅ 已完成 | 太极 **60s/圈**；L2 已移除 |
| 二 | 鱼眼状态图标（太极一体旋转） | ✅ 已完成 | **28px** 鱼眼 + R=86 + BLE 白底边框 |
| 三 | AI 运势三态 + 功能环 + 结果卡 | 🟡 代码已实现 | Animating **20s** + 12 图标环 + 触摸/Boot |
...
**★ 已废弃方案总结**：
- ❌ 8 个常驻功能图标（旧 Target1 环上八卦）：改为 **12 项 Font Awesome 运势环**（v1.12）
- ❌ 图标下方功能名字 label：v1.12 **已移除**，仅保留图标
- ❌ 环上八卦名/卦象与尺寸放大高亮：Target2 已移除，高亮改为 **圆点 + 结果卡内** 纯颜色脉冲
- ✅ Animating 态 **20s**、每 4s **0.2→0.4→0.4→0.4→0.2 圈**，恢复 **60s/圈 + 1s** 后出结果卡
- ❌ Layer2 次外细环：2026-06-15 **已移除**
- ❌ Animating 八卦环加速：Target2 无八卦环，**不再实施**
- ❌ 伪旋转（screen 固定鱼眼）：v1.7 废弃
- ❌ 太极 R=108 / 鱼眼 36px：v1.9 改为 **90% 缩放 R=86 / 28px**

---

## 当前进度报告（代码审计基线）

> **审计结论 (v1.12)**: 阶段一、二 **已实现**；阶段三运势 + **功能环** **代码已交付、真机验收中**（含音频）；阶段四至六为规划中/部分。

### 实际实现状态

| 阶段 | 内容 | 状态 | 证据 |
|------|------|------|------|
| 阶段一 基础罗盘 (Target2) | 太极 + L4 外圈（L2 已移除） | ✅ **已完成** | `attitude_display.cc` |
| 阶段二 鱼眼状态图标 | WiFi/BLE **28×28** + 太极 R=86 一体旋转 | ✅ **已完成** | MCP 手动切换；BLE 白底+边框 |
| 阶段三 AI 运势引擎 | 三态 + **12 功能环** + 触摸/Boot + 结果卡 | 🟡 **代码已实现** | `FortuneMenuType` / `ShowFortuneFromMenu()` |
| 阶段四 真实 WiFi/BLE 驱动 | esp_event → 鱼眼状态 | 🟡 **部分** | BLE server 代码已有 |
| 阶段五 AI 服务端对接 | MCP Fortune 工具 | ❌ **未开始** | 依赖阶段三验收 |
| 阶段六 增强与优化 | IMU/手势/历史/截图 MCP | 🟡 **部分** | 截图已有 |
| 外设驱动（验收迭代 5） | Codec/IMU/电量/RTC | 🟡 **部分** | **Codec + 触控已接入**；IMU/电量/RTC 未开始 |

### 已实现功能明细

- ✅ **罗盘布局（v1.12）**: 太极 R=86 + L4 r=179（358）；**12 运势图标环** r≈129
- ✅ **太极图 60s/圈**: `StartAutoRotation(60000)`，LVGL timer **150ms** 步进
- ✅ **鱼眼 28×28**: WiFi 金/灰；BLE **白底 + 边框**状态色
- ✅ **运势三态 + 功能环**: 触摸扇区 / Boot 短长按 / `ShowFortune` / Animating **20s** / Result 卡
- ✅ **板级音频**: `BoxAudioCodec`（ES8311+ES7210，PA GPIO9）；`PlayUiSound`
- ✅ **CST816S 触控**: 板级初始化 + LVGL；运势环透明触摸层
- ✅ **开机防白闪**: RGB565 `0x0841` 铺屏 + LVGL `bg_opa=COVER`
- ✅ **刷屏优化**: SPI DMA **72 行** draw buffer；运势动画收窄脏区
- 🟡 **音频真机**: 驱动已切换，**出声待验收**
- ✅ **太极 MCP 工具 ×5**: `self.attitude.taiji_*`
...
### v1.9 文档修订摘要（2026-06-15）

| 修订项 | 处理方式 |
|--------|---------|
| §2.2.2 视觉层次 | 90% 缩放；Layer2 移除；可见线环=太极边+L4 |
| §7.3 运势 Animating | 转速改为 0.2→0.4→0.4→0.4→0.2 圈/4s |
| §5 显示实现 | 宏/坐标/SetupUI 对齐 `attitude_display.h` 当前值 |
| §7.2~7.3 | 鱼眼 28px、60s/圈、BLE 白底+边框；运势 20s Animating |
| §8.2 | SPI DMA 72 行缓冲；太极 LVGL timer 150ms |
| 进度报告 | 阶段三改为「代码已实现」 |
...
| RAM (PSRAM) | ~650KB-1.2MB | 8MB | **~8-15%** | LVGL 缓冲 + 太极 canvas ~144KB (172×172 ARGB8888) |
| CPU 占用 | ~15-30% | 240MHz 双核 | **低** | LVGL 刷新 + 太极 LVGL timer 150ms + 音频处理 |
...
3. **任务优先级设计** (`FreeRTOS`):
   ...
   - **LVGL Task** (中): 刷新 UI
   - **太极旋转**: **LVGL 定时器**（非独立 FreeRTOS 任务），150ms 步进，减轻 transform 脏区刷屏
   - **Main Event Loop** (中): 事件分发/协议处理

4. **SPI 刷屏缓冲** (`lcd_display.cc`):
   - 单 DMA 内部 RAM 缓冲：**72 行** × 屏宽（原 40→60→72，减轻太极旋转撕裂）
   - 运势 Animating 仅 invalidate 必要子区域（亮度圆、外圈、鱼眼等）

5. **显示锁机制** (`DisplayLockGuard`):
...
### 已完成（阶段一 + 阶段二 + 阶段三 UI · v1.12）

- 360×360 QSPI 圆屏 + LVGL + `AttitudeDisplay`（太极 + L4 外圈 + **12 运势功能环**）
- 太极图 **172×172** canvas（R=86），**60s/圈** + WiFi/BLE **28×28** 鱼眼一体旋转
- **运势环**：Font Awesome **35px** 图标、环形触摸、Boot 短按切换/长按触发、`OGG_POPUP` 选中音
- 运势三态（Animating 20s / 结果卡）；`BoxAudioCodec` + `PlayUiSound`；CST816S 触控
- 开机防白闪（铺屏 `0x0841` + `LV_OPA_COVER`）

### 进行中 / 规划中

| 优先级 | 内容 | 参考 |
|--------|------|------|
| P1 | 运势全流程 + **音频** **真机验收** | 验收文档 **迭代 2** |
| P2 | 真实网络驱动鱼眼 | §7.4、迭代 3 |
| P3 | 服务端 Fortune MCP + 语音联调 | §7.5、迭代 4 |
| P4 | IMU / 电量 / RTC 驱动 | 迭代 5C~5E |
...
2. **迭代 2 验收**：触摸/Boot 选功能 → **20s** Animating → 结果卡；验证 `OGG_POPUP` / `OGG_SUCCESS`
3. **音频排查**：确认 ES8311/PA GPIO9 真机出声；必要时对照 Waveshare 官方示例
4. **阶段四**：真实 WiFi/BLE 驱动鱼眼（验收文档 **迭代 3**）

**预估周期**：阶段三至五约 4–5 周（不含阶段六可选项）
...
- **鱼眼图标** ×2：WiFi / BLE，**28×28**，taiji_container_ 子对象
- **运势结果卡**：200×240 胶囊形，7 行内容，Result 态显示

### A.2 Target1 状态机（运势）

```
Idle → Animating(20s) → 恢复60s/圈 → 延迟1s → Result(30s) → Idle
```
...
- ✅ Animating 态 **20s**、每 4s **0.2→0.4→0.4→0.4→0.2 圈**，恢复 **60s/圈 + 1s** 后出结果卡（迭代 2 · v1.9）
- ❌ Layer2 次外细环、10s/2+4+3+2+1 转速方案：v1.9 废弃
- ❌ 伪旋转（screen 固定鱼眼）：v1.7 废弃
- ❌ 太极 R=108 / 鱼眼 36px：v1.9 改为 **90% R=86 / 28px**

---

*文档版本: v1.12*
*修订说明: v1.12 — 12 项运势功能环、环形触摸/Boot 交互、BoxAudioCodec、CST816S、开机防白闪、PlayUiSound*
