...
> **🔴 最高优先级**: 在每一次思考和行动前，先问自己：**当前要解决的原始问题是什么？** 一切操作都必须围绕解决原始问题展开，禁止无意义的发散和探索。

#### 执行要求

1. **明确目标** - 开始任何操作前，在 TODO 列表中明确写出要解决的 1-3 个核心问题
2. **每步校验** - 每执行一个操作后，自问：这一步是否直接服务于核心问题？
3. **快速验证** - 修复完成后立即进行验证，不要在中间环节过度纠结
4. **避免完美主义** - 先让问题解决，再考虑优化；不要在修复过程中引入额外重构
